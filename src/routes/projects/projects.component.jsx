...
`);
    ...
    const fetchProjectsP = async (p) => {
        ...
        try {
            ...
            const response = await fetch(config.BASE_URL + `api/singleProjects/getPaginatedprojects?pageNumber=${p}`, { headers });
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
                    ...
                    isLoading ? (<Loader />) : (
                        projectsP.length > 0 ? (
                            ...
                            <div className='no-data'>
                                <h1>لا توجد معلومات إلى الأن، يمكنك إضافة المشاريع</h1>
                            </div>
                        )
                    )
